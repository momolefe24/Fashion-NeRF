...
def validate_batch(opt, root_opt, data,models,criterions,device,writer,loss_lrdecay=False,wandb=None, epoch=0):
    ...
    if wandb is not None:
        my_table = wandb.Table(columns=['Combined Image', 'Real Image', 'Clothing','Warped Clothing',"P Rendered","Try-On_Un", "Try-On"])
        real_image_wandb = get_wandb_image(a[0], wandb=wandb)
        clothing_image_wandb = get_wandb_image(c[0], wandb=wandb)
        warped_wandb = get_wandb_image(f[0], wandb=wandb)
        p_rendered_wandb = get_wandb_image(g[0], wandb=wandb)
...
def _train_pf_e2e_():
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        if epoch % opt.val_count == 0:
            ...
            criterions={'L1': criterionL1, 'L2': criterionL2, 'VGG': criterionVGG}
            val_loss_all, val_loss_warp, val_loss_gen, metrics = validate_batch(opt, root_opt, data,models,criterions,device,writer,loss_lrdecay=False,wandb=wandb, epoch=epoch)
