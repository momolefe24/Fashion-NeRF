...
def train_pfafn_e2e_(opt_, root_opt_, run_wandb=False, sweep=None):
...
def _train_pfafn_e2e_():
    ...
    PF_gen_model = ResUnetGenerator(opt, 7, 4, 5, ngf=64, norm_layer=nn.BatchNorm2d)
...
def validate_batch(opt, root_opt, validation_loader, PB_warp_model,PF_warp_model,PB_gen_model, PF_gen_model, total_steps,
    ...
    for i, data in enumerate(validation_loader):
        ...
        writer.add_scalar('warping_loss', loss_warp, epoch)
        ...
        if wandb is not None:
            ...
            wandb.log({'val_warping_loss':loss_warp,'val_loss_gen':loss_gen,'val_composition_loss':loss_all,'val_loss_smooth':loss_smooth,'val_loss_flow_sup_all':loss_flow_sup_all,'val_loss_l1':loss_l1,'val_loss_vgg':loss_vgg, 'val_loss_fea_sup_all':loss_fea_sup_all, 'Val_Table':my_table })
...
def train_batch(opt, root_opt, train_loader, PB_warp_model,PF_warp_model,PB_gen_model, PF_gen_model, total_steps,
    ...
    for i, data in enumerate(train_loader):
        ...
        writer.add_scalar('warping_loss', loss_warp, step)
        ...
        writer.add_scalar('loss_gen', loss_gen, step) # composition_loss
