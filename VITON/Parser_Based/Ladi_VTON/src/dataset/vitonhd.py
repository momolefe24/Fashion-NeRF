...
class VitonHDDataset(data.Dataset):
    def __init__(self,opt,root_opt,
                 dataroot_path: str,
        ...
        if phase == 'test':
            self.dataroot = self.dataroot.replace('train', phase)
        self.phase = phase
        ...
        if root_opt.dataset_name ==  'Rail':
            ...
            self.c_names = list(set(self.clothing_names))
    ...
    def __getitem__(self, index):
        ...
        if self.opt.dataset_name == 'Rail':
            clothing_index = self.c_names.index(f"{self.get_clothing_name(im_name)}.jpg")
