...
from VITON.Parser_Based.ACGPN.ACGPN_train.util.image_pool import ImagePool
...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt, wandb_opt=None):
        BaseModel.initialize(self, opt, wandb_opt)
        ...
        if self.isTrain:
            ...
            if wandb_opt is not None:
                self.old_lr = wandb_opt.lr
            else:
                self.old_lr = opt.lr
            if wandb_opt is not None:
                beta1 = wandb_opt.beta1
            else:
                beta1 = opt.beta1
            ...
            self.criterionStyle = networks.StyleLoss(opt, self.gpu_ids)
            ...
            self.optimizer_G = torch.optim.AdamW(params, lr=self.old_lr, betas=(beta1, 0.999))

            params = list(self.Unet.parameters()) + list(self.G1.parameters()) + list(
                self.G2.parameters())
            self.optimizer_G_remain = torch.optim.Adam(params, lr=self.old_lr, betas=(beta1, 0.999))

            # optimizer D
            params = list(self.D.parameters())
            self.optimizer_D = torch.optim.AdamW(params, lr=self.old_lr, betas=(beta1, 0.999))

            # optimizer D
            params = list(self.D3.parameters()) + list(self.D2.parameters()) + list(
                self.D1.parameters())
            self.optimizer_D_remain = torch.optim.Adam(params, lr=self.old_lr, betas=(beta1, 0.999))

            # load networks
            
            pretrained_path = '' if not self.isTrain else opt.g1_save_final_checkpoint_dir
            load_last_step = self.opt.load_last_step if type(self.opt.load_last_step) == bool else eval(self.opt.load_last_step)
            if load_last_step:
                self.load_network(self.Unet, opt.unet_load_step_checkpoint)
                self.load_network(self.G1, opt.g1_load_step_checkpoint)
                self.load_network(self.G2, opt.g2_load_step_checkpoint)
                self.load_network(self.G, opt.g_load_step_checkpoint)
            # self.load_network(self.D, 'D', opt.which_epoch, pretrained_path)
            elif os.path.isfile(opt.unet_load_final_checkpoint) and os.path.isfile(opt.g1_load_final_checkpoint) and os.path.isfile(opt.g2_load_final_checkpoint) and os.path.isfile(opt.g_load_final_checkpoint):
                self.load_network(self.Unet, opt.unet_load_final_checkpoint)
                self.load_network(self.G1, opt.g1_load_final_checkpoint)
                self.load_network(self.G2, opt.g2_load_final_checkpoint)
                self.load_network(self.G, opt.g_load_final_checkpoint)
    ...
    def forward(self, label, pre_clothes_mask, img_fore, clothes_mask, clothes, all_clothes_label, real_image, pose,
        ...
        # ipdb.set_trace()
        arm1_mask = torch.FloatTensor((label.cpu().numpy() == 11).astype(np.float32)).cuda()
        arm2_mask = torch.FloatTensor((label.cpu().numpy() == 13).astype(np.float32)).cuda()
        face_mask = 0#torch.FloatTensor((label.cpu().numpy() == 12).astype(np.float32)).cuda()
        ...
        fore_mask=torch.FloatTensor((label.cpu().numpy() >0).astype(np.float32)).cuda()
        ...
        CE_loss += self.BCE(fake_cl, clothes_mask) * 10 # Lambda 2

        # ipdb.set_trace()
        fake_cl_dis = torch.FloatTensor((fake_cl.detach().cpu().numpy() > 0.5).astype(np.float32)).cuda()
        new_arm1_mask = torch.FloatTensor((armlabel_map.cpu().numpy() == 11).astype(np.float32)).cuda()
        new_arm2_mask = torch.FloatTensor((armlabel_map.cpu().numpy() == 13).astype(np.float32)).cuda()
        ...
        L1_loss = self.criterionFeat(fake_image, real_image)*10 # lambda2
        #
    ...
    def save_step(self, which_epoch):
        self.save_network(self.Unet,which_epoch, self.opt.unet_save_step_checkpoint,self.opt.unet_save_step_checkpoint_dir)
        self.save_network(self.G,which_epoch, self.opt.g_save_step_checkpoint,self.opt.g_save_step_checkpoint_dir)
        self.save_network(self.G1,which_epoch, self.opt.g1_save_step_checkpoint,self.opt.g1_save_step_checkpoint_dir)
        self.save_network(self.G2,which_epoch, self.opt.g2_save_step_checkpoint,self.opt.g2_save_step_checkpoint_dir)
        self.save_network(self.D,which_epoch, self.opt.d_save_step_checkpoint,self.opt.d_save_step_checkpoint_dir)
        # self.save_network(self.D1,which_epoch, self.opt.d1_save_step_checkpoint,self.opt.d1_save_step_checkpoint_dir)
        # self.save_network(self.D2,which_epoch, self.opt.d2_save_step_checkpoint,self.opt.d2_save_step_checkpoint_dir)
        # self.save_network(self.D3,which_epoch, self.opt.d3_save_step_checkpoint,self.opt.d3_save_step_checkpoint_dir)
        # self.save_network(self.optimizer_G, which_epoch, self.opt.optimizer_G_save_step_checkpoint,self.opt.optimizer_G_save_step_checkpoint_dir,cuda=False)
        # self.save_network(self.optimizer_D, which_epoch, self.opt.optimizer_D_save_step_checkpoint,self.opt.optimizer_D_save_step_checkpoint_dir,cuda=False)

        pass

    def save(self):
        self.save_network(self.Unet,"latest", self.opt.unet_save_final_checkpoint,self.opt.unet_save_final_checkpoint_dir)
        self.save_network(self.G,"latest", self.opt.g_save_final_checkpoint,self.opt.g_save_final_checkpoint_dir)
        self.save_network(self.G1,"latest", self.opt.g1_save_final_checkpoint,self.opt.g1_save_final_checkpoint_dir)
        self.save_network(self.G2,"latest", self.opt.g2_save_final_checkpoint,self.opt.g2_save_final_checkpoint_dir)
        self.save_network(self.D,"latest", self.opt.d_save_final_checkpoint,self.opt.d_save_final_checkpoint_dir)
        # self.save_network(self.D1,which_epoch, self.opt.vgg_save_step_checkpoint,self.opt.vgg_save_step_checkpoint_dir)
        # self.save_network(self.D2,which_epoch, self.opt.vgg_save_step_checkpoint,self.opt.vgg_save_step_checkpoint_dir)
        # self.save_network(self.D3,which_epoch, self.opt.vgg_save_step_checkpoint,self.opt.vgg_save_step_checkpoint_dir)
