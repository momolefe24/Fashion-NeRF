...
class CPDataset(data.Dataset):
    ...
    def __init__(self, root_opt, opt):
        ...
        if self.datamode == 'test':
            self.read_data_dir = self.read_data_dir.replace('train', 'test')
    ...
    def __getitem__(self, index):
        ...
        if self.root_opt.dataset_name == 'Rail':
            ...
            image_mask_path = osp.join(self.read_data_dir, 'image-mask', im_name.replace("jpg","png"))
            parse_path = osp.join(self.read_data_dir, 'image-parse-new', parse_name)
            c_name = self.c_names[index]
        ...
        # im_parse = Image.open(osp.join(self.data_path, 'image-parse-new', parse_name)).convert('L')   # updated new segmentation
        parse_array = np.array(im_parse)
        im_mask = Image.open(image_mask_path).convert('L')
        ...
        parse_shape_ori = parse_shape_ori.resize(
            (self.fine_width, self.fine_height), Image.BILINEAR)
        shape_ori = self.transform_parse_shape(parse_shape_ori)  # [-1,1]
...
class CPDataLoader(object):
    def __init__(self, opt, dataset):
        ...
        self.data_loader = torch.utils.data.DataLoader(
            dataset, batch_size=opt.viton_batch_size, shuffle=(
